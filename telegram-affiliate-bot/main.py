...
AFFILIATE_TAG = os.environ.get('AFFILIATE_TAG', 'pixelofferte-21')  # solo il tag, es: pixelofferte-21
...
def extract_asin_from_amazon_path(path: str):
    ...
    if m:
        return m.group(1)
    m = re.search(r'/gp/aw/d/([A-Z0-9]{10})', path)
    if m:
        return m.group(1)
...
def build_affiliate_amazon_url(original_url: str, tag: str) -> str:
    """
    Costruisce link Amazon con il tag affiliato.
    Se trova ASIN, restituisce https://<dominio>/dp/ASIN/?tag=...
    Altrimenti aggiunge il parametro tag alla query se non già presente.
    """
    try:
        ...
        if asin:
            new_path = f'/dp/{asin}/'
            new_query = urlencode({'tag': tag})
            return urlunparse((p.scheme, p.netloc, new_path, '', new_query, ''))

        # conserva param esistenti e aggiungi tag
...
# HTTP session per risoluzione redirect short-links
HTTP_SESSION = None  # sarà creato in main()
...
async def resolve_redirect(url: str) -> str:
    ...
    try:
        ...
        logging.debug(f"Non ho potuto risolvere {url}: {e}")
        return url

async def transform_links_in_text_async(text: str):
    """
    Risolve short links e costruisce link affiliati per amazon.
    Restituisce (testo_sostituito, liste_coppie_old_new).
    """
    if not text:
        return text, []
    ...
    for orig in links:
        # rimuove punteggiatura finale preservandola
        stripped = orig.rstrip(string.punctuation)
        suffix = orig[len(stripped):]  # eventuale punteggiatura finale
        candidate = stripped

        parsed = urlparse(stripped)
        host = (parsed.netloc or "").lower()

        # se short-link amzn.to o amzn.* (non amazon), risolvi
        if 'amzn.to' in host or (host.startswith('amzn.') and 'amazon.' not in host):
            resolved = await resolve_redirect(stripped)
            candidate = resolved

        # se l'host finale è amazon.*, costruisci affiliato
        ...
        if 'amazon.' in final_host:
            ...
            if new_url and new_url != candidate:
                ...
                logging.debug(f"Transform link: {orig} -> {replacement}")
            else:
                # fallback: prova ad aggiungere tag a stripped
                fallback = build_affiliate_amazon_url(stripped, AFFILIATE_TAG)
                if fallback != stripped:
                    replacement = fallback + suffix
                    replaced = replaced.replace(orig, replacement, 1)
                    fixed_links.append((orig, replacement))
                    logging.debug(f"Fallback transform link: {orig} -> {replacement}")
        else:
            # se origin host già amazon but matched differently
            if 'amazon.' in host:
                ...
                if new_url != stripped:
                    ...
                    replaced = replaced.replace(orig, replacement, 1)
                    fixed_links.append((orig, replacement))
                    logging.debug(f"Direct amazon link fix: {orig} -> {replacement}")

    return replaced, fixed_links

async def rebuild_buttons_async(original_buttons):
    """
    Ricostruisce bottoni URL aggiornando eventuali link amazon/amzn.
    Restituisce struttura compatibile o None.
    """
    ...
    try:
        for row in original_buttons:
            ...
            for b in row:
                ...
                # alcuni bottoni possono essere in .button
                ...
                if url and text:
                    ...
                    # se non è URL-button, prova a mantenere l'oggetto
                    try:
                        new_row.append(b)
                    except Exception:
...
async def handler(event):
    try:
        ...
        # altri media (immagini, video, doc)
...
async def main():
    ...
    # avvia client e registra handler (senza await su funzioni non-awaitable)
    await client.start()
    if SOURCE_CHANNELS:
